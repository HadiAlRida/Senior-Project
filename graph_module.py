...
def bootstrap_clustering(G, n_iterations=100):
    clustering_results = []
    nodes = list(G.nodes())
    for _ in range(n_iterations):
        sampled_nodes = np.random.choice(nodes, size=len(nodes), replace=True)
        subgraph = G.subgraph(sampled_nodes)
        partition = detect_communities_louvain(subgraph)
        clustering_results.append(partition)
